...
class SecondFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        secondViewModel?.navToFirstFragment = { findNavController().navigate(R.id.firstFragment) }
        ...
        arguments?.let{
            ...
            secondViewModel?.getWeatherInCity(nameCity!!,"5add6a6a1179b651037584ea12d07431")
        }

        /** поиск погоды */
        binding.idSecondButtonSearch.setOnClickListener {
            if(binding.idSecondEt.text?.isNotEmpty() == true){
                secondViewModel?.getWeatherInCity(binding.idSecondEt.text.toString(),MY_KEY)
                binding.idSecondEt.text?.clear()
            }else{
                secondViewModel?.showToast("you did not enter the city name")
            }
        }
